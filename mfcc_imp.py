...
def loadfile(filename, data_list):
	...
	mini_val = 1000000
	for a in data_list:
		if float(a[1]) - float(a[0]) < mini_val:
			mini_val = float(a[1]) - float(a[0])
		temp1 = math.floor(samplerate*float(a[0]))
		temp2 = math.ceil(samplerate*float(float(a[1])))
		temp = [int(temp1), int(temp2), a[2]]
		if temp[1] - temp[0] < min_val:
			min_val = temp[1] - temp[0]
		l1.append(temp)
	l1 = np.array(l1)
	print "min_val recv ", min_val
	print "min a val", mini_val
	temp = l1[0]
	newWavFileAsList = []
...
def transcript(filename):
	...
	for i in range(1,len(lines)):
		...
		l = x.split(' ')
		if l[0]!='':
			if l[3]!="[MISC]" and l[3]!="(Emotional" and l[3]!="(Emotion":
...
def frame_signal(final, samplerate):
	...
	for temp in final:
		...
		z = np.zeros((int(pad_signallength - signallength)))
...
if __name__=="__main__":
	...
	for elem in mfcc:
		...
		if float(temp[0]) > 100 and float(temp[0]) < 500:
			a = mfcc1.flatten()
			if a.shape[0] >= test:
				test = a.shape[0]
			B = np.pad(a, (0, 2782 - len(a)), 'constant')
			#shapeo = pca.fit_transform(np.transpose(mfcc1))
			#print shapeo.shape

			bk = B.tolist()
			bk = ' '.join([str(x) for x in bk])
			final_list.append(tag + ' ' + bk)
